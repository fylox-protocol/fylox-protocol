...
function applyLang(code) {
  ...
  // Update all bottom nav labels across ALL screens

  document.querySelectorAll('.nl').forEach(el => {

    const txt = el.textContent.trim().toLowerCase();

    if (txt === 'home' || txt === t.home.toLowerCase()) el.textContent = t.home;

    if (txt === 'wallet' || txt === t.wallet.toLowerCase()) el.textContent = t.wallet;

    if (txt === 'earn' || txt === t.earn.toLowerCase()) el.textContent = t.earn;

    if (txt === 'profile' || txt === t.profile.toLowerCase()) el.textContent = t.profile;
  ...
  document.querySelectorAll('.ttitle').forEach(el => {
    ...
    const map = {
      ...
      'Activity': t.activity, 'Actividad': t.activity, '活动': t.activity,

      'Notifications': t.notifications, 'Notificaciones': t.notifications,

      'Wallet': t.wallet, 'Billetera': t.wallet,

      'Profile': t.profile, 'Perfil': t.profile,
