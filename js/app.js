...
function applyLang(code) {
  ...
  // Nuclear scan — catches any hardcoded text in index.html
  translateDOM(t);

  if (navigator.vibrate) navigator.vibrate(30);
}

// ─── Nuclear DOM translator ───────────────────────
// Scans ALL text nodes in the app and translates any
// that exactly match a known translation key value.
// This catches hardcoded text in index.html that has
// no data-i18n attribute.
function translateDOM(t) {
  const allValues = {};
  // Build a map: "any translated text" → translation key
  const keys = Object.keys(LANGS.en);
  Object.values(LANGS).forEach(langObj => {
    keys.forEach(key => {
      const val = langObj[key];
      if (val && typeof val === 'string') {
        allValues[val.trim().toLowerCase()] = key;
      }
    });
  });

  // Walk every element inside #app
  ...
  const walker = document.createTreeWalker(
    ...
    NodeFilter.SHOW_TEXT,
    {
      acceptNode(node) {
        ...
        // Skip if parent already has data-i18n (handled elsewhere)
  ...
  while ((node = walker.nextNode())) {
    ...
    if (txt && allValues[txt]) {
      nodesToUpdate.push({ node, key: allValues[txt] });
    }
  }
